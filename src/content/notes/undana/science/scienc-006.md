...
1. A 2 kg crate slides 5 m down a 30° frictionless incline. Using **W = F·s·cosθ**, compute the work done by gravity, then find the final speed via the work–energy theorem. (Expected: W = 49 J, v ≈ 7 m/s.)
2. A loop of area 0.02 m² rotates from perpendicular (θ = 0°) to parallel (θ = 90°) to a 0.5 T field in 0.1 s. Using **ε = −N·(dΦ/dt)** with N = 1, calculate the average induced EMF. (Expected: ε = 0.1 V.)
