...
1. Technology is held constant (no technological improvement)
2. At least one input is fixed (short-run condition)
3. All units of the variable input are homogeneous (identical quality)
4. The state of technology does not change during the period of study
...
1. **MP = dTP/dL** — the slope of the TP curve
2. When MP > 0, TP is rising
3. When MP = 0, TP is at its maximum
4. When MP < 0, TP is falling
5. **MP cuts AP from above at the maximum point of AP** — this is a fundamental geometric relationship
6. When MP > AP, AP is rising
7. When MP < AP, AP is falling
...
If inputs are multiplied by t (>1), output is multiplied by more than t.
...
**Mathematical representation:**
If inputs are multiplied by t (>1), output is multiplied exactly by t.
...
**Mathematical representation:**
If inputs are multiplied by t (>1), output is multiplied by less than t.
...
**Causes of DRS:**
1. **Management difficulties**: As scale grows, coordination and control become harder — the entrepreneur's ability to effectively supervise is stretched
2. **Bureaucracy**: Larger organizations become more bureaucratic, slowing decision-making
3. **Communication breakdown**: More layers of management distort communication
4. **Loss of personal touch**: Motivating workers becomes harder without personal relationships
5. **Logistical inefficiencies**: Larger scale may create transport and coordination bottlenecks
...
1. Downward sloping (for efficient regions) — to keep output constant, if you increase one input, you must decrease the other
2. Convex to the origin — reflects the principle of **Diminishing MRTS**
3. Higher isoquant = higher output (further from origin)
4. Isoquants cannot intersect (otherwise the same combination would produce two different outputs)
5. They are negatively sloped in the economic region
...
1. The isoquant (representing the desired output level) is **tangent** to the isocost line
2. At this tangency: **Slope of isoquant = Slope of isocost line**

Therefore:
**MRTS = w/r**

Or equivalently, using the ratio formula:
**MP_L / w = MP_K / r**
...
In long-run equilibrium (cost minimization):
**MRTS(L,K) = w/r**
...
**Upper Ridge Line:** Points where MP_K = 0 (with respect to capital, varying labour)
**Lower Ridge Line:** Points where MP_L = 0 (with respect to labour, varying capital)
...
**Minimize:** C = wL + rK
**Subject to:** Q = f(L, K) ≥ Q̄

**First-Order Condition (FOC):**
At optimum: MRTS(L,K) = w/r
Or equivalently: MP_L/MP_K = w/r

**Second-Order Condition (SOC):**
The isoquant must be convex at the tangency point (diminishing MRTS).
...
**L* = L(w, r, Q)** — demand for labour as function of input prices and output
**K* = K(w, r, Q)** — demand for capital as function of input prices and output
...
In the **Hicks-neutral** case, technical progress increases the efficiency coefficient A:
**Q = A(t) · f(L, K)** — output increases without changing the marginal rate of substitution.
...
Technical progress that increases the marginal product of labour more than capital:
**Q = f(A(t)·L, K)**

### Capital-Augmenting Technical Progress

Technical progress that increases the marginal product of capital more than labour:
**Q = f(L, A(t)·K)**
...
**FOCs:**
1. ∂ℒ/∂L = f_L – λw = 0 → f_L = λw
2. ∂ℒ/∂K = f_K – λr = 0 → f_K = λr
3. wL + rK = C

From (1) and (2): f_L/f_K = w/r → MRTS = w/r

And since λ = f_L/w = f_K/r, the condition can be written as:
**f_L/w = f_K/r**
...
1. Draw the isoquant representing the target output Q̄
2. Draw the isocost line representing the budget C: K = C/r – (w/r)L
3. The tangency point between the isoquant and the isocost line is the cost-minimizing (producer's equilibrium) point
4. At this point:
...
**Solution:**
Using the condition: MP_L/MP_K = w/r (if both variable)

But here K is fixed (short-run), so the firm just equates the value of marginal product to input price:

**VMPL = MP_L × P** (assuming output price P = Rs. 2 for illustration)
At optimum: VMPL = w
(40 – 2L) × P = 10
...
1. **Short-run vs Long-run distinction is fundamental** — the law of variable proportions governs the short run; returns to scale govern the long run.

2. **The firm should operate in Phase II** — where MP > 0 but diminishing. Phase I is inefficient underutilization; Phase III is destructive overutilization.

3. **MP cuts AP from above** — geometrically, the MP curve intersects the AP curve at the AP's maximum point.

4. **IRS → CRS → DRS** is the typical pattern as firms grow — initially economies dominate, then constant returns, then diseconomies.

5. **Isoquants are to the firm what indifference curves are to the consumer** — analogous but not identical concepts.

6. **MRTS = w/r at cost-minimizing equilibrium** — the slope of the isoquant must equal the slope of the isocost line.

7. **Ridge lines** mark the boundary where an input's marginal product becomes zero or negative.

8. **The Cobb-Douglas production function** (Q = AL^αK^β) is the most commonly used functional form — know its properties including returns to scale classification.
