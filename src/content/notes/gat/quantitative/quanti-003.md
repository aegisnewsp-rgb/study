...
**Shortcut for Equal Groups**:
$$\text{Average of Group} = \frac{\text{Sum of items in group}}{\text{Number of items}}$$
...
Example 3: Average of first n natural numbers = (n+1)/2
           (This is the middle number since they're equally spaced)
```

**Common Average Values to Memorize**:
| Numbers | Average |
|---------|---------|
| 1, 2, 3, ..., n | (n+1)/2 |
| 2, 4, 6, ..., 2n | n+1 |
...
Average speed = 3D / (11D/60) = 180/11 = 16.36 km/h
```

⚡ **GAT PYQ**: "A man covers half the distance at 12 km/h and the other
half at 18 km/h. Find his average speed." → Answer: 14.4 km/h
...
New person age = 243 - 200 = 43 years
```
...
So the remaining 4 students (not the new one) have average = (276 - 66)/4 = 52.5 kg
```

#### Temperature and Gradient Problems
...
5. A man covers 200 km at 50 km/h and 200 km at 100 km/h.
   ...
   Solution: Equal distances, so use formula:
             v_avg = 2v₁v₂/(v₁+v₂) = 2×50×100/(150) = 66.67 km/h
```

⚡ **GAT Strategy**: For average speed with equal distances, use 2v₁v₂/(v₁+v₂). For equal times, use (v₁+v₂)/2.

#### Mean Deviation and Range

**Mean Deviation**:
$$\text{Mean Deviation from Mean} = \frac{\sum |x_i - \bar{x}|}{n}$$
...
For grouped data or small samples:
