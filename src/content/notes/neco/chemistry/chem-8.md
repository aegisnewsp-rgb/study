...
diagramPrompt: "Clear scientific diagram of Acids, Bases, Salts and pH with atom labels, molecular structure, reaction arrows, white background, color-coded bonds and groups, exam textbook style"
...
*Example:* Calculate pH of $0.001$ M HCl.
$$[\text{H}^+] = 0.001 = 10^{-3} \text{ M} \Rightarrow \text{pH} = -\log(10^{-3}) = 3$$

**pOH and the Relationship:**
$$\text{pH} + \text{pOH} = 14 \text{ (at } 25°C\text{)}$$
$$\text{pOH} = -\log[\text{OH}^-]$$

*Example:* Find pH of $0.01$ M NaOH.
$$[\text{OH}^-] = 10^{-2} \text{ M} \Rightarrow \text{pOH} = 2 \Rightarrow \text{pH} = 14 - 2 = 12$$
...
*Example:* Find $[H^+]$ in $0.1$ M acetic acid ($K_a = 1.8 \times 10^{-5}$).
$$[\text{H}^+] = \sqrt{1.8 \times 10^{-5} \times 0.1} = \sqrt{1.8 \times 10^{-6}} = 1.34 \times 10^{-3} \text{ M}$$
...
For strong acids, $\alpha \approx 1$ (complete dissociation). For weak acids:
...
Before equivalence: excess $H^+$ present → pH determined by excess acid.
After equivalence: excess $OH^-$ present → determine pOH then convert to pH.

**pH of Weak Acid–Strong Base Titration:**

At equivalence point: pH > 7 (weak acid's conjugate base makes solution slightly basic).
The salt formed (e.g., CH₃COONa) hydrolyses:
$$\text{CH}_3\text{COO}^- + \text{H}_2\text{O} \rightleftharpoons \text{CH}_3\text{COOH} + \text{OH}^-$$

**Salt Hydrolysis:**

For salt of weak acid and strong base: solution is basic ($pH > 7$).
For salt of strong acid and weak base: solution is acidic ($pH < 7$).
For salt of weak acid and weak base: pH depends on $K_a$ and $K_b$:
$$\text{pH} = \frac{1}{2}\left(\text{p}K_a + \text{p}K_b\right) + \frac{1}{2}\log C$$
...
If ion product $> K_{sp}$: precipitation occurs.
If ion product $< K_{sp}$: no precipitation (unsaturated solution).
