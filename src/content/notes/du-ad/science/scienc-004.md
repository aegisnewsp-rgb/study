...
Study strategy: Focus on energy transformation diagrams for each physical situation; practice connecting force-displacement analysis with energy analysis
...
**General gravitational potential energy** (for objects far from Earth's surface where g changes appreciably):
**PE = −GMm/r**
...
**Elastic collision in one dimension** (with target initially at rest):
After collision, velocities depend on the mass ratio. For equal masses, velocities simply exchange.
...
**Work stored in a spring:**
The work required to stretch or compress a spring by distance x from equilibrium equals ½kx². This equals the elastic potential energy stored in the spring: PE_spring = ½kx².
...
1. **Work from force at angle:** Calculate work given force magnitude, displacement, and angle
2. **Work-energy theorem:** Given initial conditions, apply W = ΔKE to find unknown velocity or displacement
3. **Conservation of mechanical energy:** Set KE + PE initial = KE + PE final; solve for velocity at different heights
4. **Power calculations:** P = W/t for simple cases; P = Fv when velocity is known
5. **Spring problems:** Combine Hooke's Law with energy conservation — PE_spring = ½kx² converts to KE or gravitational PE
6. **Collision problems:** Identify collision type; apply momentum conservation always, KE conservation only if elastic; determine final velocities
