...
1. **Mass units:** Always convert mass to kilograms before using F = ma. Using grams gives answers 1000 times too large.
2. **Directional signs:** Momentum and velocity are vectors. Establish a positive direction and apply it consistently.
3. **Third-law pairs:** Action and reaction never act on the same body. When a block rests on a table, the weight of the block and the normal reaction are a third-law pair only if you trace them correctly — both act on the block.
4. **Apparent weight in lifts:** Remember the lift's acceleration adds to g when going up, subtracts when coming down.
