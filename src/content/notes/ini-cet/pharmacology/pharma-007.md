...
Phospholipase A₂ (from cell membrane phospholipids) → Arachidonic acid
...
1. **GI toxicity** (most common — COX-1 inhibition): Gastric ulcers, bleeding — PGE₂ is gastroprotective
2. **Renal toxicity**: ↓ prostaglandins → ↓ renal blood flow → acute kidney injury, interstitial nephritis (especially with chronic use)
3. **Bleeding** (non-selective NSAIDs): ↓ platelet aggregation → ↑ bleeding time (COX-1 inhibition in platelets → ↓TXA₂)
4. **CV risk** (COX-2 selective): ↑ thrombotic events (COX-2 inhibition → ↓ PGI₂ without ↓TXA₂ → net prothrombotic state) — Vioxx (rofecoxib) withdrawn for ↑MI risk
5. **Asthma exacerbation**: Some patients (especially with nasal polyps) — cross-sensitivity with aspirin (leukotrienes path not blocked)
6. **Aplastic anemia**: Phenylbutazone (withdrawn) — bone marrow suppression
