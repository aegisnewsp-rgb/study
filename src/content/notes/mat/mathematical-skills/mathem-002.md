...
**Basic Arithmetic Operations** — Quick Facts
...
Always solve in this sequence: **B**rackets → **O**f (powers) → **D**ivision → **M**ultiplication → **A**ddition → **S**ubtraction
...
**Basic Arithmetic Operations** — Deep Dive
...
When all values increase by same amount $k$, average increases by $k$.
When some values are removed, adjust sum accordingly.
...
**Basic Arithmetic Operations** — Complete Theory
...
1. **Estimate first**: Round numbers to nearest 10/100, compute approximate answer, then check if exact answer is in same ballpark.
2. **Digit-sum check**: For addition, sum digits of addends and compare to digit-sum of result. For multiplication, digit-sum of product equals digit-sum of product of digit-sums.
3. **Divide to verify multiply**: If $a \times b = c$, then $c \div b = a$ exactly.
4. **Cross-check with inverse**: Use subtraction to verify addition, division to verify multiplication.
