...
- This is calculated as: `ITC reversal = ITC claimed × (remaining useful life / total useful life)`.

**Example:**
A machine was purchased for Rs. 10 lakhs with ITC of Rs. 1.8 lakhs (18% GST). Useful life is 10 years. If sold after 3 years:
...
1. The tax must be **actually payable** by the supplier.
2. The tax must be ** charged** on the invoice.
3. The ITC cannot exceed the tax amount actually payable.

**Credit Note and Reduction of ITC:**
If a credit note is received from the supplier (for discount, return, or price reduction):
...
1. The vehicle is used for **further supply** (i.e., as stock-in-trade for reselling).
2. The vehicle is used for **transport of passengers** (public transport, taxi, bus).
3. The vehicle is used for **imparting driving skills** (driving schools).
4. The vehicle is used for **transport of goods** (goods carriage).
5. The vehicle is a **running vehicle** (on road) used for **towing services**.
...
1. The goods are **written off** due to **natural calamity/fire** (not willful negligence).
2. The goods are **lost in transit** (documented with carrier records).
3. The goods are **written off** as part of the **normal business process** (e.g., sample testing, quality control).
...
**Example:**
A Head Office in Delhi receives a single invoice from a CA firm for **audit services**. The audit service benefits all branches across India. The Head Office, as an ISD, will distribute the ITC from this invoice to each branch based on their turnover or other parameter.
...
**How It Works:**
1. The ISD receives **consolidated invoice** for input services.
2. The ISD **allocates** the ITC to each business unit (cost centre/branch/division).
3. The ISD must file **GSTR-6** (return for ISD) monthly.
4. The ITC is **credited** to the electronic credit ledger of each unit.
...
**Example:**
A small restaurant (not serving alcohol) with annual taxable turnover of Rs. 60 lakhs:
- GST payable = 5% of Rs. 60 lakhs = Rs. 3 lakhs
- No ITC can be claimed for any purchases

**Key Condition — No Inter-State Supplies:**
Composition suppliers **cannot make inter-State supplies** of goods. If they do, they are **exited from the composition scheme** and become regular taxpayers.
...
Composition suppliers must file **GSTR-9A** annually.
...
1. **Export under Bond/LUT** (Letter of Undertaking) — No GST paid on export. ITC can be claimed on inputs. Refund of accumulated ITC.
2. **Export with IGST payment** — GST paid on export at 0% (claimable as refund immediately).
...
**Refund Amount in Inverted Duty Structure:**
...
2. Inverted duty structure must exist.
3. ITC must have accumulated for at least **Rs. 1,000** in the tax period.
4. Refund application must be filed within **2 years** from the end of the financial year.
...
1. **File GSTR-3B and GSTR-1** for the relevant tax period.
2. **Calculate refund amount** in the electronic cash ledger.
3. **File Refund Application — FORM GST RFD-01** (or through common portal).
4. **Acknowledgement** issued within 15 days (acknowledgement of complete application).
5. **Processing** by refund officer within **60 days** of acknowledgement.
6. **Sanction order** issued with refund amount.
...
If refund is not disbursed within **60 days** from the date of acknowledgement:
...
In fraud cases, the **goods** involved may be **confiscated** and penalty imposed.
...
**Recovery Process:**
1. Order passed confirming demand.
2. Taxpayer given time to pay (usually 30 days).
3. If not paid → **Recovery Certificate** issued.
4. Recovery officer can:
   - Sell goods/property
...
**Purpose:**
To **protect revenue** — prevent the taxpayer from dissipating assets during investigation or dispute.

**Process:**
1. Commissioner passes a **speaking order** (with reasons).
2. Attachment is **provisional** — pending final determination of tax liability.
3. **Opportunity of hearing** must be given (unless urgent — can be immediate with post-decision hearing).
4. **Review** by the Commissioner within 30 days.
...
1. **ITC Conditions [Section 16]:** Must be registered, receive goods/services, use for business, tax paid by supplier, valid invoice in possession, supplier has filed return.

2. **Blocked Credits [Section 17(5)]:** Motor vehicles for personal use, food and beverages (except resale), health services, club memberships, travel benefits, lost/stolen goods, composition taxpayers.

3. **Composition Scheme [Section 10]:** 1% (manufacturing), 5% (restaurants), 3% (other services) — no ITC available, quarterly filing GSTR-4.

4. **E-Way Bill:** Mandatory for inter-State movement (all values); intra-State movement when value > Rs. 50,000.

5. **Invoices:** Tax Invoice (taxable supply), Bill of Supply (exempt/composition), Receipt Voucher (advance), Debit/Credit Note (adjustments), Revised Invoice (errors).

6. **Refund [Section 54]:** Export refunds (zero-rated), inverted duty structure (limited), deemed exports, excess payment — file RFD-01.

7. **Demands:** Section 73 (no fraud, 3 years, 10% penalty), Section 74 (fraud, 5 years, 100% penalty).

8. **Anti-Evasion:** Section 83 (provisional attachment), Section 67 (search and seizure), Section 132 (prosecution for serious evasion).

9. **Appeals:** First appeal to Appellate Authority (3 months, 10% pre-deposit), second appeal to CESTAT (6 months, 20% pre-deposit).

10. **Advance Ruling:** Available for classification, rate applicability, ITC eligibility — not for composition taxpayers.
...
1. Discuss the conditions for claiming input tax credit under Section 16 of the CGST Act. What are the circumstances under which ITC would be denied?

2. Explain the blocked credits under Section 17(5) of the CGST Act with specific examples. When can ITC on motor vehicles be claimed?

3. What is the composition scheme under Section 10? Explain the eligibility criteria, rates, and filing requirements for composition taxpayers.

4. Explain the e-way bill rules under Rule 138 of the CGST Rules. When is e-way bill mandatory and what is the validity period?

5. A company has accumulated input tax credit because the tax rate on inputs (18%) is higher than the tax rate on outputs (5%). Can they claim refund? Explain with reference to Section 54.

6. Distinguish between Section 73 and Section 74 of the CGST Act with reference to penalty, time limit, and fraud provisions.

7. What is the difference between a tax invoice, bill of supply, receipt voucher, and credit note? When is each issued?

8. Explain the provisions relating to provisional attachment under Section 83 and search and seizure under Section 67 of the CGST Act.
