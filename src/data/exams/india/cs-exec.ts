...
const exam: ExamTemplate = {
  ...
  examPattern: `Under the ICSI New Syllabus 2022 the CS Executive Programme has 7 papers in 2 groups, each paper of 3 hours and 100 marks. Group 1: Jurisprudence, Interpretation & General Laws (JIGL); Company Law & Practice; Setting Up of Business, Industrial & Labour Laws (SBILL); and Corporate Accounting & Financial Management (CAFM). Group 2: Capital Market & Securities Laws (CMSL); Economic, Commercial and Intellectual Property Laws (ECIPL); and Tax Laws and Practice. All papers except CAFM use a 20% case-based objective plus 80% descriptive pattern; CAFM is fully descriptive. To pass, a candidate must score at least 40% in each paper and 50% in aggregate — taken across both groups together, or across one group at a sitting when that module is attempted alone. A paper passed with 60% or more carries an exemption into later attempts, but only where the candidate appeared in every paper of that module and scored at least 25% in each of the others.`,
  eligibility: `Candidates who have passed the CSEET (CS Executive Entrance Test) are eligible to register for the CS Executive Programme. ICSI also admits candidates directly without CSEET: those who have passed the ICSI Foundation Examination, the final stage of CA (ICAI) or the final stage of CMA (ICMAI), and graduates and postgraduates (for the commerce-stream graduate route, more than 50% aggregate across the three years). Final-year graduation students may register provisionally subject to submitting proof of passing. CSEET itself is being restructured from the June 2026 session, so candidates should confirm the current entrance requirement against ICSI's latest announcement.`,
  subjects,
  durations: {
