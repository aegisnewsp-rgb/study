...
- For a vehicle with **constant power P** and mass m starting from rest: v = √(2Pt/m) (so v ∝ √t), a = P/(mv)
- ⚡ **Derivation:** P = Fv = m(dv/dt)v → ∫v dv = (P/m)∫dt → ½v² = (Pt)/m → v² = (2Pt)/m → **v = √(2Pt/m)**. Equivalently, the work-energy theorem W = Pt = ½mv² gives the same result. Velocity grows as √t, and acceleration a = P/(mv) decreases as the vehicle speeds up.
- ⚡ **Dimension check:** Pt/m has units of J/kg = m²/s², so √(2Pt/m) correctly has units of m/s.
