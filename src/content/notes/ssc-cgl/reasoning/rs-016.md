...
• A cube made of **small unit cubes** (cubelets) has side length $n$. It contains:
  ...
  - Cubes with **2 faces painted** = $12 \times (n-2)$
...
1. **Two Opposite Faces Painted** (e.g., top and bottom):
   Opposite faces of a cube share no edge and no corner, so no small cube can touch both painted faces. Every small cube therefore has either exactly one painted face or none:
   - Cubes with exactly 1 painted face: all $n^2$ cubes on each painted face = $2n^2$
   - Cubes with 2 or 3 painted faces: $0$
   - Unpainted cubes: $n^3 - 2n^2$
   Contrast with **two adjacent faces painted**: the shared edge gives $n$ cubes with exactly 2 painted faces (including its two corner cubes, since only these two faces carry paint), and the remaining $n^2 - n$ cubes on each face have exactly 1 painted face = $2(n^2 - n)$.
...
- Only the left and right faces carry paint, and no unit cube touches both (opposite faces share no edge or corner).
- Every one of the $n^2 = 16$ cubes on each painted face has exactly 1 painted face — including the edge and corner positions of that face, because the adjoining faces of the big cube are unpainted.
- So: $2n^2 = 16 + 16 =$ **32 cubes** have exactly 1 painted face
