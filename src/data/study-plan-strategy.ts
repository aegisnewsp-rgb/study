...
export function getStudyPlanStrategy(opts: {
  ...
  const byTier: Record<ArchetypeId, Tier> = {
    cram: {
      ...
      whatToCut: `Study only ${examName}'s weight-5 topics — for most candidates the heaviest of ${topProse}. Everything weight-4 and below is noise at this range; skip it without guilt.`,
      mockPlan: `No full mocks. Spend every minute on previous-year ${examName} questions for your highest-weight topics and memorise their solution patterns.`,
      weeklyRhythm: `There is no week — work in 90-minute blocks on your heaviest ${examName} topics, short breaks between, prioritising recall over re-reading.`,
    ...
    sprint: {
      ...
      whatToCut: `Cover ${examName}'s weight 4–5 topics properly, starting with ${topProse}. Touch weight-3 topics only if you finish early; skip weight 1–2 entirely.`,
      mockPlan: `Sit two or three timed previous-year ${examName} papers in the second half and review every wrong answer the same day.`,
      weeklyRhythm: `Front-load new ${examName} learning into the first 60% of days; reserve the last 40% for previous-year papers and error review.`,
    ...
    intensive: {
      ...
      whatToCut: `Cover ${examName}'s weight 3–5 topics thoroughly — ${topProse} first. Give weight 1–2 topics a single light reading in your final week rather than skipping them outright.`,
      mockPlan: `From the second week, sit one full-length ${examName} mock every week and analyse it fully before moving on — analysis matters more than the score.`,
      weeklyRhythm: `Each week: 5 days new ${examName} topics, 1 day consolidating that week, 1 day mock + review. Keep a running error log.`,
    ...
    focused: {
      ...
      whatToCut: `Cover the entire ${examName} syllabus once, then let weightage — led by ${topProse} — decide what earns a second and third pass. Nothing is skipped, only deprioritised.`,
      mockPlan: `Topic-wise ${examName} tests while you learn, then weekly full-length mocks once the first pass is done. Track sectional timing, not just the total.`,
      weeklyRhythm: `Roughly the first 60% of the timeline on the first pass of the ${examName} syllabus, the next 25% on weight-prioritised revision, the last 15% on full mocks and an error-log review.`,
    ...
    foundation: {
      ...
      whatToCut: `Cover everything, and give ${examName}'s weight 3–5 topics — ${topProse} above all — a second problem-solving pass. Low-weight topics get one solid pass; at this length they are worth keeping, not cutting.`,
      mockPlan: `Topic and sectional ${examName} tests through the build phase; full-length mocks every other week from the midpoint, weekly in the final two months. Maintain an error log from the start.`,
      weeklyRhythm: `Three arcs across the ${examName} syllabus: a concept-building phase, a depth-and-problems phase, and a revision-plus-mocks phase. Each subject gets at least two spaced passes.`,
    ...
    longhorizon: {
      ...
      whatToCut: `Cut nothing. Over a year, ${examName}'s low-weight topics — even beyond ${topProse} — are exactly where you build the edge most candidates never reach; depth compounds at this length.`,
      mockPlan: `Light topic tests in the first months, monthly full-length ${examName} mocks from the midpoint, shifting to weekly in the final 10–12 weeks. Revisit your error log on a spaced schedule throughout.`,
      weeklyRhythm: `Quarter-by-quarter through the ${examName} syllabus: foundations, depth and problem-solving, full-syllabus revision, then a mock-and-fine-tuning quarter. Re-touch every subject at least three times.`,
    ...
    marathon: {
      ...
      whatToCut: `Nothing is cut and nothing is rushed. Across two years the differentiator is depth on ${examName}'s hardest, lowest-frequency topics — well beyond ${topProse} — and relentless revision, the work most candidates skip.`,
      mockPlan: `Year one: topic and sectional ${examName} tests only, building accuracy. Year two: monthly then fortnightly then weekly full-length mocks, with a disciplined error log you actually revisit.`,
      weeklyRhythm: `Think in semesters, not weeks: build, deepen, revise, simulate the ${examName} syllabus — repeated across two cycles so every subject is seen many times on a spaced schedule.`,
